...
The first step is to install AWS Parallelcluster console by according to the following guide: https://docs.aws.amazon.com/parallelcluster/latest/ug/install-virtualenv.html

Once you have AWS Parallcluster installed (and activated ig you are installing it in a Python Virtual Env), the next step is to 
...
**warning**  
* Several of these settings will result in higher cost.  Please review [EC2 costs](https://aws.amazon.com/ec2/pricing/) prior creation creation.  
...
post_install_args = https://github.com/usumfabricae/hpc-workshop-wrf.git,setup.sh 
...
Load environmet variables related to WRF set-up:
```bash
...
### Explore results
After forecast is completed we can have a look at WRF's output files using ncview, a graphical application allowing visualization of WRF output. 
...
This commands deletes the Master node and you will loose any forecast data.
Master node can also be simply switched off when not used and switched on again 
